...
const checkFolderExists = async (id_carpeta) => {
    try {
        ...
        const result_folder = await client.execute("SELECT * FROM Carpeta WHERE id_carpeta = ?", [id_carpeta]);
...
const checkListExists = async (id_lista) => {
    try {
        ...
        const result_list = await client.execute("SELECT * FROM Lista_reproduccion WHERE id_lista = ?", [id_lista]);
...
const checkListBelongsToFolder = async (id_lista, id_carpeta) => {
    try {
        ...
        const result_list = await client.execute("SELECT * FROM Listas_de_carpeta WHERE id_carpeta = ? AND id_lista = ?", [id_carpeta, id_lista]);
        return result_list.rows.length > 0;
...
// Verifica si el contenido multimedia es un episodio o una canción
